...
pigeonpea_mwi_val_prod_all=pigeonpea_mwi["pigeonpea_val_prod_all"]

plot(pigeonpea_mwi["pigeonpea_val_prod_all"],main="Pigeonpea value (US$)")

## pp yield
pigeonpea_yield <- crop_spam("pigeonpea", "yield", path="raster", TRUE)

pigeonpea_yield_mwi <- mask(pigeonpea_yield ,malawi_aoi)
pigeonpea_yield_mwi <- crop(pigeonpea_yield_mwi,malawi_aoi)

pigeonpea_yield_mwi_all=pigeonpea_yield_mwi["pigeonpea_yield_all"]

plot(pigeonpea_yield_mwi["pigeonpea_yield_all"],main="Pigeonpea yield (kg/ha)")

## pp harvested area
pigeonpea_harv_area <- crop_spam("pigeonpea", "harv_area", path="raster", TRUE)

pigeonpea_harv_area_mwi <- mask(pigeonpea_harv_area ,malawi_aoi)
pigeonpea_harv_area_mwi <- crop(pigeonpea_harv_area_mwi,malawi_aoi)

pigeonpea_harv_area_mwi_all=pigeonpea_harv_area_mwi["pigeonpea_harv_area_all"]

plot(pigeonpea_harv_area_mwi["pigeonpea_harv_area_all"],main="Pigeonpea harvested area (ha)")
...
pigeon_pea_suit_prodn=terra::extract(pigeonpea_mwi_val_prod_all,vect(landunits),fun=mean)
pigeon_pea_suit_yield=terra::extract(pigeonpea_yield_mwi_all,vect(landunits),fun=mean)
pigeon_pea_suit_harv_area=terra::extract(pigeonpea_harv_area_mwi_all,vect(landunits),fun=mean)

landunits$pigeonpea_val_prod_all=pigeon_pea_suit_prodn$pigeonpea_val_prod_all
landunits$pigeonpea_yield_all=pigeon_pea_suit_yield$pigeonpea_yield_all
landunits$pigeonpea_harv_area_all=pigeon_pea_suit_harv_area$pigeonpea_harv_area_all

# Value of production regression

pp_suit_value_reg=lm(pigeonpea_val_prod_all~T_Pigeonpe_suit_num,data=landunits)
summary(pp_suit_value_reg)

library(visreg)
visreg(pp_suit_value_reg)

# Yield model
pp_suit_yield_reg=lm(pigeonpea_yield_all~T_Pigeonpe_suit_num,data=landunits)
summary(pp_suit_yield_reg)

library(visreg)
visreg(pp_suit_yield_reg)

# Area model
pp_suit_harv_area_reg=lm(pigeonpea_harv_area_all~T_Pigeonpe_suit_num,data=landunits)
summary(pp_suit_harv_area_reg)

library(visreg)
visreg(pp_suit_harv_area_reg)
...
PP_Errorplot=
  landunits%>% 
  drop_na(T_Pigeonpe_suit) %>%
  ggerrorplot(x = "T_Pigeonpe_suit", y = "pigeonpea_harv_area_all",add = "mean", error.plot = "errorbar", color="black",size=1, ggtheme=theme_bw())+
  labs(x="Pigeon pea suitability",y="Current harvested area (ha)")+
  theme_bw(base_size = 16)+coord_flip()
PP_Errorplot

library(ggplot2)

landunitspp=landunits%>% 
  drop_na(T_Pigeonpe_suit)

PP_sum_plot=  ggplot(landunitspp, aes(T_Pigeonpe_suit, pigeonpea_harv_area_all)) +
